...
def test_datetime_range_where_sql_empty():
    """datetime_range_where_sql returns empty when both bounds None."""
    out = datetime_range_where_sql(None, None, "dt")
    assert out == ""


def test_datetime_range_where_sql_both_bounds():
    """datetime_range_where_sql produces [after, before) with AND."""
    out = datetime_range_where_sql(
        "2020-01-01",
        "2020-12-31",
        "created_at",
    )
...
def test_datetime_range_where_sql_only_after():
    """datetime_range_where_sql with only datetime_after."""
    out = datetime_range_where_sql("2020-06-01", None, "dt")
    assert out == "\"dt\" >= '2020-06-01'"


def test_datetime_range_where_sql_only_before():
    """datetime_range_where_sql with only datetime_before."""
    out = datetime_range_where_sql(None, "2021-01-01", "ts")
    assert out == "\"ts\" < '2021-01-01'"


def test_datetime_range_where_sql_iso_format():
    """datetime_range_where_sql accepts ISO datetime strings."""
    out = datetime_range_where_sql(
        "2020-01-15T10:30:00",
        "2020-01-15T18:00:00",
        "updated_at",
