"""Functional tests for FalkorDB connector.

This module provides comprehensive functional tests that validate the FalkorDB
...
Run all functional tests::
    ...
    pytest test/db/falkordbs/test_functional.py -v

Run specific category::

    pytest test/db/falkordbs/test_functional.py -v -k "DataCoherence"
...
- test.db.falkordbs.test_edge_cases : Technical edge case tests
- test.db.falkordbs.test_performance : Performance and load tests
"""
...
class TestETLScenarios:
    ...
    def test_case_sensitivity(self, conn_conf, test_graph_name, clean_db):
        ...
        with ConnectionManager(connection_config=conn_conf) as db:
            ...
            result = db.fetch_docs("User")
            # FalkorDB is case-sensitive by default
            assert len(result) >= 1
    ...
    def test_mixed_type_timestamps(self, conn_conf, test_graph_name, clean_db):
        ...
        with ConnectionManager(connection_config=conn_conf) as db:
            ...
            result = db.execute(
                "MATCH (e:Event) RETURN e.id, e.created_at ORDER BY e.created_at"
            )
            assert len(result.result_set) == 3
...
class TestDataQuality:
    ...
    def test_date_format_variations(self, conn_conf, test_graph_name, clean_db):
        ...
        with ConnectionManager(connection_config=conn_conf) as db:
            ...
            result = db.execute("MATCH (e:Event) RETURN e.id, e.date ORDER BY e.date")
