"""Utility ontology classes for resource connectors and configurations.

This module provides Pydantic models for managing resource connectors (files and database tables)
and configurations used throughout the system. These classes support resource discovery,
connector matching, and configuration management.

Key Components:
    - ResourceConnector: Abstract base class for resource connectors
    - FileConnector: Configuration for file connector matching
    - TableConnector: Configuration for database table connector matching
    - Bindings: Collection of named resource connectors with connection management
"""
...
class Bindings(ConfigBaseModel):
    ...
    def from_dict(cls, data: dict[str, Any] | list[Any]) -> Self:
        ...
        if (
            ...
            # Strip __tag__ from nested connector dicts so extra="forbid" does not fail
